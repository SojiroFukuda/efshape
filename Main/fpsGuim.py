...
class MyForm(Qw.QMainWindow):               
    ...
    def onStringChanged(self,value):
        # Log #--------------
        self.ui.logbox.append(datetime.now().strftime("%Y/%m/%d %H:%M:%S"))
    ...
    def onStringChangedPCA(self,value):
        # Log #--------------
        self.ui.logbox_pca.append(datetime.now().strftime("%Y/%m/%d %H:%M:%S"))
    ...
    def setFPSPath(self):
        ...
        self.ui.textbox_fpsPath.setText(FPS_FOLDA+os.sep+self.SAVE_FILE_NAME)
        self.FPS_PATH = FPS_FOLDA+os.sep+self.SAVE_FILE_NAME
    ...
    def checkPCASetting(self):
        ...
        if self.SAVE_PCA == "":
            ...
            if N > sumple_num:
                checker = True
                # Log #--------------
                File_log_str += "ERROR: Total number of sumples must be larger than Maximum Harmonics" + "\n"
                # -------------------
        if checker:
